      ...
      <div className="space-y-4">
        {orders.map(o=>(
          <div key={o.id} className="border rounded-xl p-4">
            ...
            <div className="mt-2 text-sm text-gray-600">{new Date(o.createdAt).toLocaleString()}</div>
            <ul className="mt-3 text-sm list-disc ml-5">
              {o.items.map(i=>(
                <li key={i.id}>{i.qty} × {i.product.name} {i.variant?.size? `(Size ${i.variant.size})`:''}</li>
              ))}
            </ul>
          </div>
        ))}
